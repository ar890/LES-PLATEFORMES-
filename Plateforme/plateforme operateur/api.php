<?php
// Désactiver l'affichage des erreurs et activer la journalisation des erreurs
ini_set('display_errors', 0); 
ini_set('log_errors', 1);     
ini_set('error_log', '/path/to/your/php-error.log'); // Mettre à jour le chemin du fichier log
...
function getTicketsInstallation($pdo) {
    $stmt = $pdo->query("SELECT * FROM tickets WHERE statut = 'installation'");
    $tickets = $stmt->fetchAll(PDO::FETCH_ASSOC);
    sendResponse($tickets);
}

function getActivationsService($pdo) {
    $stmt = $pdo->query("SELECT * FROM activations WHERE statut = 'service'");
