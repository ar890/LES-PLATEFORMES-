document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderClientList(clientData = clients) {
        ...
        document.querySelectorAll('.status-select').forEach(select => {
            select.addEventListener('change', updateClientStatus);
        });
    }
    ...
    function renderTicketList(ticketData = tickets) {
        ...
        ticketData.forEach((ticket, index) => {
            ...
            row.innerHTML = `
                ...
                <td>${ticket.statut}</td>
                <td>
                    <select data-index="${index}" class="ticket-status-select">
    ...
    function updateClientStatus(event) {
        const select = event.target;
        const index = select.dataset.index;
        clients[index].statut = select.value;
        renderClientList();
    }
